...
import { assistantMessages, windowMessages, terminalBootLines } from './core/poke-assistant-messages.js';
...
class Desktop95 {
  constructor() {
    ...
    this.pokeVaultQuest = new PokeVaultQuest({
      playClickSound: () => this.playClickSound(),
      showBotAssistant: (message) => this.botAssistant.show(message)
    });
    this.pokeVaultQuest.registerTerminalCommands(this.terminal);
  ...
  init() {
    ...
    setTimeout(() => {
      this.botAssistant.show(assistantMessages[0]);
  ...
  handleWindowOpened(windowId) {
    setTimeout(() => {
      const msg = windowMessages[windowId];
  ...
  setupTerminal() {
    ...
    for (const line of terminalBootLines) {
      this.terminal.terminalPrint(line, true);
    }
